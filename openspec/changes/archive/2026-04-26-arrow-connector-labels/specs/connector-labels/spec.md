...
A patchDiagram arrow connector MAY include a text label using Mermaid pipe syntax. When a label is present in a full connection (`sourceId:Port -->|label text| destId:Port`), the diagram SHALL render the label string along the connector wire.
...
A patchDiagram MAY declare a dangling arrow that terminates at a port with a label at the open (source) end, using the syntax `-->|label text| destId:Port`.
...
### Requirement: Dangling-from connection with label (existing behavior preserved)
The existing `sourceId:Port -->|label|` syntax SHALL continue to produce a dangling stub originating from the source port with the label at the open end.
