...
**Reason**: The two-rectangle header/body node design is replaced by the banded-frame style. There is no longer a separate "header" region; `primaryColor` now maps to the overall node background colour (`nodeBgColor`).
**Migration**: Set `patch.nodeBgColor` directly, or set `themeVariables.primaryColor` which now maps to `nodeBgColor`.

---

### Requirement: Mermaid secondaryColor theme variable sets the node body background

**Reason**: The node body rectangle is eliminated by the banded-frame redesign. `secondaryColor` no longer has a node-box target.
**Migration**: No direct equivalent. Use `patch.nodeBandLight` / `nodeBandMid` / `nodeBandDark` to control band colours explicitly.

---

### Requirement: Mermaid primaryBorderColor theme variable sets the node border colour

**Reason**: The node box no longer has a single border stroke; the visual border is formed by the outermost band pair. `primaryBorderColor` now maps to `nodeBandDark` (the outermost/darkest band).
**Migration**: Set `patch.nodeBandDark` directly, or set `themeVariables.primaryBorderColor` which now maps to `nodeBandDark`.
