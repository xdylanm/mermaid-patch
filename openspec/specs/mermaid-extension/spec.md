...
The renderer SHALL read color values from the resolved Monotrail configuration, which is derived by merging the package defaults with any overrides supplied under the `monotrail` key in `mermaid.initialize()`. When Mermaid's `theme` option is set to `'dark'`, the renderer SHALL automatically apply dark-mode color overrides without requiring the user to specify colors explicitly.
...
- **WHEN** `mermaid.initialize({ theme: 'dark' })` is called with no `monotrail.dark` overrides
- **THEN** the renderer SHALL automatically apply built-in dark-mode defaults (e.g. `audioColor: '#f4a7c3'`, `background: '#1e1e2e'`)
...
#### Scenario: Per-color override in dark mode

- **WHEN** `mermaid.initialize({ theme: 'dark', monotrail: { dark: { audioColor: '#ff88aa' } } })` is called
- **THEN** only `audioColor` SHALL be overridden in the dark palette; all other dark-mode colors SHALL use their built-in dark defaults
